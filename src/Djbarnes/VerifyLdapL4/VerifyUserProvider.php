<?php
namespace Djbarnes\VerifyLdapL4;
...
class VerifyUserProvider implements UserProviderInterface
{
     ...
     * @return void
     */
    public function __construct($model, $ldapserver)
    {
        $this->model = $model;
        //$this->hasher = $hasher;
    ...
    public function validateCredentials(UserInterface $user, array $credentials)
    {
        ...
        if(ldap_start_tls($connection));
          if(!$result = @ldap_bind($connection,$user->userdn,$credentials['password']))
            throw new UserPasswordIncorrectException('Username or Password is incorrect');
